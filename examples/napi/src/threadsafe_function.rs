...
pub fn call_threadsafe_function(callback: JsFunction) -> Result<()> {
  let tsfn: ThreadsafeFunction<u32, ErrorStrategy::CalleeHandled> = callback
    .create_threadsafe_function(0, |ctx| {
      println!("callback entered from user-end");
      println!("ctx value {}", ctx.value);
...
extern "C" fn __napi__call_threadsafe_function(
  ...
  unsafe {
    println!("call threadsafe function");
...
//       println!("callback entered from user-end");
//       println!("ctx value {}", ctx.value);
//       println!("ctx env napi version {:?}", ctx.env.get_napi_version());
//       ctx.env.create_uint32(ctx.value + 1).map(|v| vec![v])
//     })?;
//   // println!("tsfn {:#?}", tsfn.raw_tsfn);
//   // for n in 0..100 {
//   //   let tsfn = tsfn.clone();
//   //   thread::spawn(move || {
//   //     tsfn.call(Ok(n), ThreadsafeFunctionCallMode::Blocking);
